...
(
cd ../
cp build/libs/sor-szallitas-dronokkal*.jar docker/production/sor-szallitas-dronokkal-customer.jar
)

#creating the production image from the copied jar file
docker build -t zoltanaty/sor-szallitas-dronokkal-customer ./production || exit 1

#removing the jar file from the 'production' folder
rm -rf production/*.jar

#pushing the created image to dockerhub
docker push zoltanaty/sor-szallitas-dronokkal-customer
